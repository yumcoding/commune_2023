...
import StarRatingBtn from "@/components/common/StarRatingBtn";
import { useSession } from "next-auth/react";
import postMyReview from "@/app/actions/postMyReview";
...
export default function ReviewWriteModalContent({ isModal }: { isModal: boolean }) {
	...
	async function onPost() {
		...
		// const res = await postMyReview(formData);
		if (searchData && title?.length > 0 && content?.length > 0 && session?.data?.user) {
			...
			formData.append("bookIsbn", searchData.isbn);
			formData.append("bookTitle", searchData.title);
			formData.append("bookImage", searchData.image);
			formData.append("bookAuthor", searchData.author);
			formData.append("userId", session?.data?.user?.id);

			await postMyReview(formData);
		}
	}
	...
	return (
		<>
			<section className={cls(wrapper, isModal ? "" : isPage)}>
				...
				<div className={formWrapper}>
					<form action={onPost}>
						<input type="text" placeholder="리뷰 제목을 정해주세요:)" name="title" value={title} onChange={onChangeTitle} />
						<div className={textareaWrapper}>
							<textarea placeholder="책에 대해 자유롭게 의견을 남겨주세요!" name="content" value={content} onChange={onChangeContent}></textarea>
							<p className={counter}>{content?.length} / 1000</p>
						</div>
						<div className={flexbox}>
							<div className={ratingWrapper}>
